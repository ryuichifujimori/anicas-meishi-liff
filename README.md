...
- 右下に `public/anicas_logo_br_square.png` を canvas 合成（中央固定の
  `image` オプションは使わない）。白い丸ごとQRの右下隅に内接し、3つの
  ファインダパターン・タイミングパターン・形式情報には掛からない。
- 白い丸の直径は `lib/meishi-layout.ts` の `LOGO_DISC_MM`（**6.4mm**）。
  丸が隠したモジュールは誤り訂正レベルHが復元するので、**大きくできる上限は
  「まだ読めるか」で決まる**。7.4mm はどの解像度でも読めず、6.8mm は14文字の
  ハンドルで150dpiを落とす。6.4mm は30文字までのどのハンドルでも従来と同じ
  読み取り性能。変更するときは `docs/print-quality-verification.md` と同じ表を
  取り直すこと。
