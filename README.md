...
- 2匹以上のとき、ステップ4に「名前の間隔」のバーが出る。バーは列の間隔を動かし、
  種類は名前の軸に乗っているので必ず一緒に動く。
- **バーの可動域は固定値ではない**（`spreadLimits`）。入力された名前を実際に
  測って毎回決める。右端＝名前がカードの左右の余白に届くところ、左端＝名前の
  文字送りの箱どうしが接するところ。プレビューは canvas、入稿PDFは fontkit で
  測るので、片方だけ範囲が違うことはない。
