...
- さわれるのは **5つだけ**：写真のかたまり／種類の行／名前の行／Instagram の行
  （印＋アカウント名）／QR（ロゴ含む）。オーナー行・リボン・絵柄は動かない。
- できるのは **動かす／大きさを変える** の2つだけ。触ると選ばれて枠が出て、
  角に大きさのつまみが4つ出る。枠の外を触ると外れる。覚える状態は
  「いまどれを選んでいるか」の1つだけ。
- 位置と大きさは `lib/card-adjust.ts` が**面ごとのまとまり**として持つ
  （`{ front: { photo, breed, name, ig, qr } }`）。プレビューと入稿PDFは
...
| 写真 | 下端がリボンの帯の下端（`RIBBON_BAND.bottom`）を越えない＝帯からはみ出さない |
| 文字 | 設計上いちばん小さい級数 `MIN_TYPE_SIZE`（＝種類の 0.033＝約1.82mm＝5pt強）より小さくならない |
| QR | 1モジュールが `MIN_QR_MODULE_MM`（0.30mm）より小さくならない。QRの密度に応じて自動で決まる |

Instagram の印はテンプレート画像に焼き込まれているため、行を動かしたときだけ
元の印を白で伏せ、同じ画素を透明背景に取り出した `public/meishi-instagram.png`
を置き直す。**触っていなければ従来どおりテンプレートの印がそのまま出る**ので、
何もしなければ入稿PDFは1画素も変わらない。

「最初に戻す」は1つだけ置いてある。押すと5つとも既定の位置と大きさに戻る。

実測値・比較画像・QR デコード結果:

- `docs/direct-edit-verification.md` … 最新（プレビュー上の直接編集・はみ出し・最小サイズ）
...
```json
{
  ...
  "adjust": {
    "front": {
      ...
      "breed": { "dx": 0, "dy": 0, "scale": 1 },
      "name":  { "dx": 0, "dy": 0, "scale": 1 },
      "ig":    { "dx": 0, "dy": 0, "scale": 1 },
...
- `adjust` … タレントがプレビューの上で動かした位置と大きさ。面ごと（いまは
  `front` のみ）に、5つの部分それぞれの `dx`／`dy`（カード幅・高さに対する割合）と
  `scale`（既定の何倍か）。何も触っていなければ全部 `dx:0, dy:0, scale:1`。
  作り直しのときに同じレイアウトで組み直せるように残す。GAS 側で使わないなら
  無視してよい。**`name_spread` は廃止**（「名前の間隔」のバーごとなくなった）。
