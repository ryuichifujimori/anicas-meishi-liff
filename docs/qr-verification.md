...
- 白円・ロゴは **ファインダグリッド上の右下（= 4 つ目の目の位置）に整列**。
  中心 = `origin + (moduleCount − 3.5) × dotSize`（左上原点はマトリクス中央寄せ
  `origin = floor((width − count·dotSize)/2)`）。これにより
  **右上ファインダと同じ横位置・左下ファインダと同じ高さ**になる。
- 縁は canvas の `arc` 塗りで antialias（ジャギーなし）。
- ロゴが白円内に収まるよう `LOGO_RATIO` を `0.12` に調整（マークが白円からはみ出さない）。
- 白円・ロゴはいずれも 3 つの実ファインダパターンには被らない。

**ピクセル計測（実レンダリング、`/tmp/qr_generated.png` を connectedComponents で計測）:**

```
左上ファインダ外枠 : 182 × 182 px   （= 7 × dotSize, moduleCount=37, dotSize=26）
右下の白円         : 180 px（179×181）   円/目 = 0.989 → 差 −1.1%（±5%以内）✅

整列（中心座標）:
  右上ファインダ x = 890.0  /  白円 x = 889.5  → 差 −0.5px（横位置一致）✅
  左下ファインダ y = 890.0  /  白円 y = 889.5  → 差 −0.5px（高さ一致）  ✅
```
