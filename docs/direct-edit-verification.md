...
この回で足したのは3つ——**写真の下端をリボンに差し込んだまま切ること**（6章）、
**中央や等間隔でぴたっと止まり、そのときだけ細い縦線が出ること**（7章）、
**出たときに動かせる部分が一度だけ光ること**（8章）。あとの2つはどちらも
一時的な表示で、画面に残る要素は増えていない（10章）。
...
{ "photo": { "scale": 0.920, "dx":  0.020, "dy":  0.015 },
  ...
  "pets":  [ { "scale": 1.188, "dx": -0.019 },
             { "scale": 0.912, "dx":  0.040 },
             { "scale": 1.332, "dx":  0.036 } ] }
```

（列の値が指を離した位置ちょうどでないのは、7章の「ぴたっと止まる」が
効いているため。しまわれた値がそのまま入稿PDFに出ていることが以下の実測。）
...
| 3匹 | 1匹目 | 2匹目 | 3匹目 |
|---|---|---|---|
| 種類（プレビュー / PDF） | 2.156 / 2.156 | **1.800 / 1.800** | 2.418 / 2.418 mm |
| 名前（プレビュー / PDF） | 4.965 / 4.965 | 3.811 / 3.811 | 5.568 / 5.568 mm |

（2匹目の種類が 1.800mm ちょうどなのは、縮めていって種類の下限で止まったところ。
名前だけがそこから先も縮む——4章。）
...
リボンの重ね絵 `/meishi-ribbon.png` は、テンプレートの白を透明に抜いて作ってある。
そのためリボン自身の**白い塗りも一緒に抜けていて**、両端のひらひらは線だけで中が
透ける。写真はリボンの下に描かれるので、そこへ届いた写真はひらひらを素通しで
見えてしまう。

帯が実際に覆っている幅をファイルから実測すると——

| リボンの行 | 途切れずに不透明な範囲（1046px 中） | カード比 |
|---|---|---|
| 850 行（いちばん広い） | px 56 … 990 | 0.0535 … 0.9474 |
| 870 行 | px 64 … 982 | 0.0612 … 0.9398 |
| 922 行 | px 86 … 960 | 0.0822 … 0.9187 |
| 936 行より下 | 中央も透明 | — |

写真の枠は px 52 … 994（0.05 … 0.95）なので、**帯が写真を左右いっぱいに覆う行は
1つもない**。デザインどおりの下端（`PHOTO_TUCK` = 0.50）がすでに限界で、
そこから下げると角が出る。旧実装は帯の下端（0.53855）まで下げられたため、
角が 3.5mm ぶん顔を出していた。

### 直しかた

止める位置を変えるだけでは足りない（写真は横にも動くし大きくもなるので、
下端の露出は左右の広がりでも変わる）。そこで**下端より下は描かない**ことにした。
プレビューは `clip-path: inset()`、入稿PDFはクリップパスで、どちらも同じ
`PHOTO_TUCK` で切る。**下端に届いていない写真には切り取り自体を出さない**ので、
触っていないカードは1画素も変わらない（→ 7章）。

### 実測

写真を**つまみで最大まで大きくし、指で最も下まで動かして**送信した。

| | 写真の枠（プレビュー実測） |
|---|---|
| 左右 | 0.000 … 55.000 mm（カード幅いっぱい＝拡大の上限） |
| 上下 | 1.383 … 49.006 mm（下端の止まり＝`RIBBON_BAND.bottom`） |

その状態で、**触っていないカードと同じ入稿PDFを 1200dpi で描画し、写真の下端の線
より下だけを画素ごとに突き合わせた**。2枚の違いは写真だけなので、下に差があれば
それが「はみ出した写真」になる。

| | 改修前（PR #10 の直前） | 改修後 |
|---|---|---|
| 1匹 | 最も深いところで **3.528 mm** / 58,845 画素 | **0.014 mm** / 264 画素 |
| 2匹 | **3.528 mm** / 63,576 画素 | **0.014 mm** / 313 画素 |
| 3匹 | **3.528 mm** / 63,281 画素 | **0.014 mm** / 313 画素 |

改修後に残る 0.014mm は 1200dpi の**走査線1本**——切り取りの境目そのもので、
その1行を除けば下は触っていないカードと完全に同一。プレビューでも同じ測りかたで
**3.585 mm → 0.076 mm**（360px 幅・3倍描画の1画素半）。

![写真の下端](direct-edit-photo.png)

リボンの帯のところだけを引き伸ばすと、何が変わったのかがはっきりする。

![リボンの帯のところ](direct-edit-photo-band.png)

---

## 7. 中央・等間隔でぴたっと止まる

止まる位置は `lib/card-adjust.ts` の **`SNAP_TABLE` という1つの表**に持たせた。
横に動かしているあいだ、表のどれかに **1.5mm** より近づくとそこで止まり、
その位置に細い縦線が出る。**線は指を離すと消える。**

| 表の行 | 止まる位置 |
|---|---|
| カードの中央 | 27.500 mm |
| 隣とのすき間が等しくなるところ | 左右にあるもの（隣の列、なければ 2.0mm の余白）のちょうど真ん中 |

対象は**列・Instagram の行・QR**。写真だけ対象外で、唯一左右の余白の外まで
届いてよい部分なので揃える相手がない。

実際に指で引いて、**止まった位置と縦線の位置を DOM から実測**した。

| 操作 | 止まった中央 | 縦線 |
|---|---|---|
| 1匹：列を中央の 2.2mm 手前まで | 25.298 mm（**止まらない**） | なし |
| 1匹：列を中央の 0.55mm 手前まで | **27.499 mm** | 27.500 mm |
| 　　同上・指を離した後 | 27.499 mm | **なし** |
| 2匹：QR を中央の 0.44mm 手前まで | **27.499 mm** | 27.500 mm |
| 2匹：1匹目を等間隔の 0.23mm 手前まで | **15.718 mm** | 15.719 mm |
| 3匹：2匹目を中央の 0.5mm 手前まで | **27.499 mm** | 27.500 mm |
| 3匹：2匹目を等間隔の 0.2mm 手前まで | **29.370 mm** | 29.372 mm |

（中央 27.500mm に対する 0.001mm のずれは、360px 幅のプレビューで CSS が
百分率を 1/64 画素に丸めるぶん。）

3匹で等間隔に止めたあとの、列と列のすき間の実測は
**左 7.185 mm / 右 7.183 mm（差 0.002 mm）**。

線が本当に消えていることは、スクリーンショットの画素からも確かめた
（アプリの緑 `#2D6A4F` がカードの高さいっぱいに並ぶ列を探す）:

| 画像 | 緑の縦列 |
|---|---|
| 中央で止まった瞬間 | x = 0.4991 … 0.5000（＝カードの中央） |
| **指を離した後** | **なし** |
| 等間隔で止まった瞬間 | x = 0.5324 … 0.5333（＝29.37mm） |

![中央線と等間隔](direct-edit-snap.png)

**ドラッグ面では文字を選択させない。** 揃って止まるとカードの見た目が動かなくなり、
その間にブラウザ自身の**文字選択**が育つ。次に指を下ろすとそれが「選択のドラッグ」
として持って行かれ、`pointercancel` で部品が動かなくなる（実測で確認した）。
編集レイヤーに `select-none` を足して、ブラウザ側のジェスチャが始まらないように
してある。

---

## 8. 動かせることが分かる — 最初に一度だけ光る

プレビューが出たとき、動かせる部分の枠が一度だけ短く光る（`HINT_MS` = 1.1秒）。
**説明文は足していない。**

| | 出た直後 | 1.6 秒後 |
|---|---|---|
| 1匹 | 光る枠 **4** 個（写真・列1・Instagram・QR） | **0** 個 |
| 2匹 | 光る枠 **5** 個 | **0** 個 |
| 3匹 | 光る枠 **6** 個 | **0** 個 |

枠は DOM から消えるので、画面に残る要素は増えていない。

![最初の光り](direct-edit-hint.png)
...
ペットの列はプレビューの上の当たり判定が匹数ぶんに分かれるだけで、
中央線と最初の光りはどちらも一時的な表示なので、画面の要素は PR #10 から
変わっていない（:4602 と :4598 で `input[type=range]` 0本・見出し・ボタンとも一致）。
