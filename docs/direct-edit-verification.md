...
（この記録は前回までのぶんも含む。今回変えたのは**写真の編集画面を消して、写真の調整を
名刺プレビューの中だけで完結させたこと**で、0 章に足してある。その前は**写真の大きさを
角のつまみからピンチに変えたこと**（0-2 章）、**スマホの指で操作できるようにしたこと**
（0-3 章）、**写真の窓を動かなくしたこと・QR と Instagram の行をさわれなくしたこと**
（0-4 章）。）
...
## 0-4. さらに前：窓は動かない、さわれるのは写真とペットの列だけ
