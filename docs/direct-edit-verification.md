...
（この記録は前回までのぶんも含む。今回変えたのは**写真の大きさを角のつまみからピンチに
変えたこと**だけで、0 章に足してある。その前は**スマホの指で操作できるようにしたこと**
（0-2 章）、さらにその前は**写真の窓を動かなくしたこと・QR と Instagram の行をさわれなく
したこと**（0-3 章）。）
...
## 0-3. その前：窓は動かない、さわれるのは写真とペットの列だけ
