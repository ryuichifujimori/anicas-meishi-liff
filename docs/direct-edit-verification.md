...
した改修の検証記録。さわれるのは **写真／ペットの列（匹数ぶん）** の2種類で、写真は指2本で大きさを変え、
列は下の「文字の大きさ」で揃える。ペットの列は1匹につき1つ、種類と名前がセットで
入っている。列は**横に動かすだけ**で、大きさはプレビューの下の「文字の大きさ」で
全部の列いっしょに決める。「名前の間隔」と「ペットNの拡大率」のバーはこの改修で
削除している。

（この記録は前回までのぶんも含む。今回変えたのは**切り抜きをリボンの端で止めたこと**で、
0 章に足してある。その前は**写真の編集画面を消したこと**（0-2 章）、**写真の大きさを
つまみからピンチに変えたこと**（0-3 章）、**スマホの指で操作できるようにしたこと**
（0-4 章）、**写真の窓を動かなくしたこと・QR と Instagram の行をさわれなくしたこと**
（0-5 章）。）
...
写真の下端の左右の端が、リボンの輪郭より外に出て見えていた。

### リボンの先より外側には、隠すものが何も無い

依頼は「リボンの先の形をなぞって斜めに切る」だったが、**なぞる先が無い**。実測すると:

| | 実測 |
|---|---|
| リボンが不透明な列 | **56〜990**（= 2.945〜52.108 mm） |
| 左の先（列56）の位置 | **どの行でも列56がリボンのいちばん左**。先より下では、ひらひらは右へ戻っていく（行852で列56、行856で58、行860で59、行896で75） |
| 写真の窓 | 2.750〜52.250 mm |

つまり **2.750〜2.945 mm と 52.108〜52.250 mm には、どの高さにもリボンが無い**。そこに
写真がある限り、下端をどんな形に切っても写真は開けたところに立っている。斜めに切っても
細くなるだけで、無くなりはしない。

**実物の名刺がその答えを持っていた。** `public/sample-meishi.png` の写真は、どの行でも
**列55〜990（2.892〜52.108 mm）**——リボンの範囲そのもの。窓の .05／.95（2.750〜52.250 mm）
は丸めた設計値で、実物より左右に 0.15〜0.19 mm 広い。その広い分がはみ出していた。

### 直しかた

**切り抜きの型紙の左右を、リボンが届く範囲そのものにした。** 型紙は
`lib/ribbon-profile.ts` に `RIBBON_SPAN`（0.053537〜0.947419）として書き出してあり、
輪郭の線もその範囲だけを走る（先より外の「平らな延長」はやめた）。

- 写真の枠（`LAYOUT.photo`）は **2.750〜52.250 mm のまま**——変えていない
- 上端・下端の設計値も変えていない
- 変わったのは**切り抜き**だけ。リボンが届かない列では写真を描かない

### 実測（入稿PDF・1200dpi・1ページ 2882×4583 画素）

写真の画素は**色で見分けていない**。同じPDFの**写真画像だけ白に差し替えて**もう一度描き、
2枚が違う画素＝写真が置いた画素、としている。

| | リボンの外に残る写真 | うち、リボンが届かない列 | 描かれた左右 |
|---|---|---|---|
| 改修前・既定（1/2/3匹とも） | **31,746 画素** | 31,746 | 2.736〜52.264 mm |
| 改修前・区画を最大にして動かした | **31,746 画素** | 31,746 | 2.736〜52.264 mm |
| **改修後・既定（1/2/3匹とも）** | **0 画素** | **0** | **2.926〜52.116 mm** |
| **改修後・区画を最大にして動かした** | **0 画素** | **0** | **2.926〜52.116 mm** |

描かれた左右の 2.926／52.116 mm は、リボンの範囲 2.945／52.108 mm に切り抜きのふちが通る
画素1つぶん（1200dpi で 0.021 mm）を足した値。

すき間のほうも塞がったまま:

| | 写真とリボンの間の白 | リボン直上 1.27mm の白い紙 | 重なり |
|---|---|---|---|
| 改修前・改修後とも、既定／最大、1/2/3匹 | **0 画素** | **0 画素** | 1〜81 画素（0.021〜1.715 mm） |

写真の枠とリボン・文字の位置は動いていない:

| | 写真の枠（配置命令から実測） | リボン・文字・QR・Instagram の配置命令 |
|---|---|---|
| 改修前 | 2.750〜52.250 mm | — |
| **改修後** | **2.750〜52.250 mm**（同じ） | 8件／10件／12件 **完全一致** |

![写真の左下と右下](direct-edit-corners.png)

---

## 0-2. 前回：写真の編集画面をなくし、名刺の上で区画ごとに直す

写真の調整は名刺プレビューの**上**にもう1つ画面があってそこでやっていた。名刺の上で
直せるものが増えたいま、同じことを2か所でやる意味がない。**編集画面を丸ごと消し、
写真の調整を名刺プレビューの中だけで完結**させた。

### 直しかた

名刺の写真の窓は、**匹数ぶんの区画に左右へ均等に分けられる**——編集画面がやっていた
分け方そのまま。区画ひとつひとつが、写真やペットの列と同じ「さわれる部分」になる:

| | 指1本 | 指2本 |
|---|---|---|
| 写真の区画（匹数ぶん） | その区画の絵が動く | その区画の絵が大きくなる |
| ペットの列（匹数ぶん） | 列が動く | なし |

1匹なら区画は1つ＝写真ぜんたい、これまでと同じ。

絵の合成そのもの（透明な地に、区画ごとに cover-fit で置く）は`lib/photo-slots.ts` に
そのまま移した——**背景を透明にする作りも、区画の分け方も、置き方も変えていない**。
変わったのは合成した絵の置き場所で、名刺プレビューが**その canvas を直接カードに描く**。
入稿PDFと payload に渡すデータURLは、指が止まって 150ms 経ってから作る（指を動かして
いるあいだ毎回 PNG に焼くのは重いため）。

窓じたいは**動きも広がりもしない**。区画の中で絵が動いて大きくなるだけなので、どの区画を
どう動かしても窓の外には出ない——構造として出ようがない。

**先読み**：区画の値は `FaceAdjust.photos` に**区画ごとの配列**で持つ。「写真を入れ替え
たい」が来たときに替わるのは元の絵だけで、区画の形も値の持ち方も変わらない。

### 実測（画面 390×844・DPR 3・タッチ。マウスは1回も使っていない）

写真の中心に**黒い四角**を置いた見本を使う。合成された canvas の中でその四角がどこに
どの大きさで現れるかが、その区画の絵が動いた量と大きくなった倍率そのもの。

| | 写真の区画 | 指2本 | 指1本 | そのあいだ他の区画が動いた量 |
|---|---|---|---|---|
| 改修前 1匹/2匹/3匹 | **1 個**（写真はひとかたまり） | — | — | — |
| **改修後 1匹** | **1 個** | **1.605 倍** | −3.382, +4.577 mm | — |
| **改修後 2匹** | **2 個** | 区画1・2とも **1.595 倍** | −3.383, +4.618 mm ／ −3.383, +4.577 mm | **0.0000 mm** |
| **改修後 3匹** | **3 個** | 区画1・2・3とも **1.595 倍** | −2.723, +4.618 mm ／ −2.723, +4.577 mm ×2 | **0.0000 mm** |

区画を1つ動かしているあいだ、他の区画は **0.0000 mm** ——1画素も動いていない。

### 編集画面が消えたこと

| | 見出し | 画面の要素 | ページの高さ |
|---|---|---|---|
| 1匹 | 写真の編集／名刺プレビュー → **名刺プレビュー** | 76 → **71** | 1584 → **1500 px** |
| 2匹 | 同上 | 86 → **79** | 1612 → **1500 px** |
| 3匹 | 同上 | 94 → **87** | 1612 → **1500 px** |

（「文字の大きさ」はラベルなので見出しの数には入れていない。バーは 1 本のまま。）

![編集画面が消えた](direct-edit-composer.png)

### 窓の外に出ない・入稿PDFと一致する・既定は変わらない

区画を全部いちばん大きくして動かしてから送信した入稿PDFを 1200dpi で描き、**同じPDFの
写真画像だけ白に差し替えて**もう一度描いた2枚の差＝写真が置いた画素、として数えた。

| | 窓の外 | 絵の箱 | 写真の画素数 |
|---|---|---|---|
| 1匹 / 2匹 / 3匹 | **0 / 0 / 0 画素** | 2.750〜52.250 ／ 2.457〜44.805 mm | **4,359,794**（3つとも同じ） |

絵の箱も画素数も**既定のときとまったく同じ**——窓は動かず、変わったのは窓の中身だけ。

| | プレビュー | 入稿PDF | ずれ |
|---|---|---|---|
| 1匹 / 2匹 / 3匹 | 2.749〜52.247 ／ 2.457〜44.805 mm | 2.750〜52.250 ／ 2.457〜44.805 mm | **0.0034 mm** |

触っていない既定の入稿PDFは、直前のコミット(9a5daaf)と**1200dpi でページ全面を1画素ずつ
突き合わせて 0 / 0 / 0 画素差**（各 13,208,206 画素中）。合成の中身を変えていないので、
既定では前とまったく同じ絵が出る。
...
角のつまみで写真の大きさを変えられることに**気づかれない**。つまみは実寸で3ミリほどの
丸で、そこを狙って引く操作は、教わらなければ思いつかない。

### 直しかた

**写真の上で指2本を広げる／狭めると、窓の中の絵が大きく／小さくなる。** 指1本でなぞる
のは今までどおり絵が動く。**角のつまみは消した。** 画面に足したものは無い——ピンチは
指の動きなので、説明も印も要らない。

| | 指1本 | 指2本 |
|---|---|---|
| 写真 | 窓の中の絵が動く | **窓の中の絵の大きさが変わる** |
| ペットの列 | 列が動く | なし（大きさは「文字の大きさ」のバー） |
| 名刺の外 | ページがスクロールする | — |

窓（描かれる範囲）は今までどおり**動きも広がりもしない**。

2本目の指が置かれると、いま持っている部分がそのままピンチに変わる。読み取るのは
**2本の指の中点・間隔・傾き**の3つで（`Grip`）、大きさに使うのは間隔だけ。**傾きも
最初から読んでいる**ので、「二本指で回したい」が来たときに足すのは `onMove` の1行で、
指の受け方そのものは変えなくていい。

指を1本離してもう1本が残ったときは、残った指の位置から測り直して「動かす」に戻る——
絵が飛ばないため。

### 実測（画面 390×844・DPR 3・タッチ。マウスは1回も使っていない）

指の動きは Chrome 自身の入力経路（CDP `Input.dispatchTouchEvent`）に流していて、2本目の
指も本物の2点目として届く。1匹・2匹・3匹とも同じ数字。

| 指の動き | 改修前 | 改修後 |
|---|---|---|
| 指2本を**狭める**（間隔 0.36 → 0.24 カード幅） | 49.498×42.348 → 49.498×42.348 mm（**1.000 倍**＝変わらない） | 49.498×42.348 → **33.000×28.230 mm（0.667 倍）** |
| 指2本を**広げる**（間隔 0.24 → 0.36 カード幅） | 49.498×42.348 → 49.498×42.348 mm（**1.000 倍**） | 33.000×28.230 → **49.498×42.348 mm（1.500 倍）** |
| 選んだ枠についているもの | **つまみ 4 個** | **0 個** |

指の間隔を 0.667 倍にすると絵も 0.667 倍、1.5 倍にすると 1.5 倍——**指の間隔そのまま**。
改修前は指2本では 1.000 倍、つまり何も起きなかった。

指1本のほうも今までどおり:

| | 前 | 後 | 動いた量 | ページのスクロール |
|---|---|---|---|---|
| 指1本で写真をなぞる | 2.749, 2.650 mm | 0.000, 6.866 mm | **−2.749 / +4.215 mm** | 485 → 485（**変わらない**） |
| 指1本で列をなぞる（1匹） | 種類 21.643 mm | 27.788 mm | **+6.145 mm** | 変わらない |
| 指1本で列をなぞる（2匹・3匹） | — | — | **+3.104 mm** | 変わらない |
| **名刺の外をなぞる** | — | — | — | **485 → 280（スクロールした）** |

![つまみが消えた](direct-edit-pinch.png)

### 指2本でいちばん大きくしても、窓の外には出ない

指2本を広げきってから送信した入稿PDFを 1200dpi で描き、**同じPDFの写真画像だけ白に
差し替えて**もう一度描いた2枚の差＝写真が置いた画素、として数えた。

| | 窓の外 | 絵の箱 | 写真の画素数 |
|---|---|---|---|
| 1匹 / 2匹 / 3匹 | **0 / 0 / 0 画素** | 0.000〜55.000 ／ 1.969〜49.023 mm | **4,359,794**（3つとも同じ） |

絵の箱はカードの端まで広がっているのに、描かれた画素数は既定のときと同じ 4,359,794。
窓は動いても広がってもいない。

触っていない既定の入稿PDFは、直前のコミット(c1ca25d)と**1200dpi でページ全面を1画素ずつ
突き合わせて 0 / 0 / 0 画素差**（各 13,208,206 画素中）。

---

## 0-4. さらに前：スマホの指で操作できるようにする
...
## 0-5. さらに前：窓は動かない、さわれるのは写真とペットの列だけ
...
選ぶと点線が実線になる。（**つまみは今回なくなった**——写真の大きさは指2本で変える。
→ 0章）
